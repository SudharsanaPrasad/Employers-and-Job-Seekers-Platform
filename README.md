...
- I used **MongoDB** because the user, job and application data is mostly separate and
  fits well as documents. I also put a unique index on (jobId, seekerId) so that a
  seeker cannot apply to the same job twice.
- The **JWT** stores the user id and the role. `@PreAuthorize` is used on the
  controllers so some endpoints work only for employers and some only for job seekers,
  and the services also check that the employer owns the job.
- All the **SMS code is kept in one service class**. If the Twilio keys are not set the
  app still runs and only prints the message in the log, so I could test everything
  without an SMS account.
- The project asked for **Twilio**, so I used Twilio. For Indian numbers MSG91 is also
  a common option, and because the SMS code is in one class it would be easy to change.
